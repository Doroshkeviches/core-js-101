...
function removeFirstOccurrences(str, value) {
  ...
  const leng = value.split('').length;
  return str.slice(0, index) + str.slice(index + leng, str.length);
}
...
function getRectangleString(width, height) {
  ...
  for (let i = 1; i <= width1; i += 1) {
    probel += ' ';
    widthLine += '─';
  }
  for (let i = 1; i <= height1; i += 1) {
    visota += `│${probel}│\n`;
  }
  const str = `┌${widthLine}┐\n${visota}└${widthLine}┘\n`;
  return str;
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < arr3.length; i += 1) {
    if (arr3[i] === ' ' || arr3[i] === '?' || arr3[i] === '!') {
      arrLast.push(arr3[i]);
    } else {
      index = arr1.indexOf(arr3[i]);
      arrLast.push(arr2[index]);
    }
  }
  return arrLast.join('');
}
...
function isString(value) {
  const a = value;
  if (typeof (a) === 'string' || Object.prototype.toString.call(value) === '[object String]') {
    return true;
  }
  return false;
}
